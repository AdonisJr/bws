...
<div class="mt-4">
    <div class="section-label">Select Discount</div>
    <select class="form-control" id="discountSelect" name="discount_id">
        <option value="" selected>No discounts at this time</option>
        <?php foreach ($discounts as $discount): ?>
            <?php if (strtotime($discount['end_time']) > time()): ?>
                <option value="<?php echo $discount['id']; ?>">
                    <?php echo $discount['service_name'] . ' - ' . $discount['discount_percentage'] . '% (Valid from ' . $discount['start_time'] . ' to ' . $discount['end_time'] . ')'; ?>
                </option>
            <?php endif; ?>
        <?php endforeach; ?>
    </select>
